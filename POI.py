...
if __name__ == "__main__":
    ...
    while True:
        ...
        if draw_centroids:
            #print(f"Centroids: {bboxes[0]}")
            for i in range(centroids.shape[0]):
                cv2.circle(	img, centroids[i,:].astype(np.uintc), 10, (0,0,255) ,2 )
        cv2.putText(img, "%.2fft" % (inches/12), (20, 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5,(0, 0, 255), 2)
        cv2.imshow('img', img)
        ...
        if len(bboxes) > 0:
            ...
            for bbox in bboxes:
                ...
                if bbox[3] > maxbox[3]:
                    maxbox = bbox
            print(maxbox)
        #If there is more than one bbox returned, and the x value (bbox[0][0]) >0, then calculate the distance to the camera
        if(len(bboxes) >0 and maxbox[3]) != 0:
            inches = distance_to_camera(6.0, focalLength, maxbox[3])
